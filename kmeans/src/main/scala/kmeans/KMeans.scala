...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = 
  	if (!points.isEmpty) points.groupBy(findClosest(_, means))
	  else means map {(_, points)} toMap
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = 
   	for {
   		p <- oldMeans
   	} yield findAverage(p, classified(p))

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
  	var i = 0;
  	var isConverged = true
  	while (i < oldMeans.length && isConverged) {
  		if (newMeans(i).squareDistance(oldMeans(i)) > eta)
  			isConverged = false
  		i += 1
  	}
  	isConverged
