...
package object scalashop {
  ...
  def boxBlurKernel(src: Img, x: Int, y: Int, radius: Int): RGBA = {
    ...
    while (row <= rowMax) {
      ...
      while (col <= colMax) {
        rv += red(src(col, row))
        gv += green(src(col, row))
        bv += blue(src(col, row))
        av += alpha(src(col, row))
        col += 1
      }
      row += 1
    }

    val numElements = (rowMax-rowMin+1) * (colMax-colMin+1)
    rgba(rv/numElements, gv/numElements, bv/numElements, av/numElements)
  }
